<?
	...
	if($_POST){
		...
		$email_body = "You have received a new message from the LifeStoriesCompany.com contact form: \n\r Name: $name \n\rEmail: $email \n\rTelephone: $telephone";
		$headers = "From: noreply@lifestoriescompany.com \n";
		$headers .= "Reply-To: $email";
		$valida = mail($to,$email_subject,$email_body,$headers);
		if($valida)
    ...
    <body>
					...
					<div class="item <?= ($key < 1 ? 'active' : '') ?>" height="500px">
						<?if($splashes->GetCampoValue($key, 'type') == 'one-photo'){?>
							<div class="col-md-12 bg-img" style="position: relative; top: 30%;background-image: url('updater/site_files/<?= $splashes->GetCampoValue($key, 'image1') ?>');background-position: center center; border-right: 2px solid #137FC8;opacity: 1;background-size: auto 100%;">
								<div class="carousel-indicators" style="position: relative; top: 30%;">
									...
									<?if($splashes->GetCampoValue($key, 'button1_text')){?>
										<a class="btn btn-lg btn-info" href="<?= $splashes->GetCampoValue($key, 'button1_link') ?>"><?= $splashes->GetCampoValue($key, 'button1_text') ?></a>
									<?}?>
								</div>
							</div>
							<?}else{?>
							<div class="col-md-6 bg-img bg-img-two" style="background-image: url('updater/site_files/<?= $splashes->GetCampoValue($key, 'image1') ?>');background-position: center center; border-right: 2px solid #137FC8;background-size: auto 100%;">
								...
								<div class="carousel-indicators" style="position: relative;top: 30%;">
									<?if($splashes->GetCampoValue($key, 'button1_text')){?>
										<a class="btn btn-lg btn-info" style="background-color: #308E2B; border: 0px solid #37B131;" href="<?= $splashes->GetCampoValue($key, 'button1_link') ?>"><?= $splashes->GetCampoValue($key, 'button1_text') ?></a>
									<?}?>
								</div>
							</div>
							<div class="col-md-6 bg-img bg-img-two" style="background: url('updater/site_files/<?= $splashes->GetCampoValue($key, 'image2') ?>') center center no-repeat;background-size: auto 100%;" id="teste">
								...
								<div class="carousel-indicators" style="position: relative;top: 30%;">
									<?if($splashes->GetCampoValue($key, 'button2_text')){?>
										<a class="btn btn-lg btn-info" style="background-color: #308E2B; border: 0px solid #37B131;" href="<?= $splashes->GetCampoValue($key, 'button2_link') ?>"><?= $splashes->GetCampoValue($key, 'button2_text') ?></a>
									<?}?>
								</div>
							</div>
						<?}?>
		...
		<div id="services">
			<div class="container">
				...
				<div class="row">
					<?php foreach ($service->mItems as $key => $value) {?>
		...
		<div id="portfolio">
			<div class="container">
				...
				<div id="slide-portfolio-memoirs" class="carousel slide" data-ride="carousel" data-interval="false">
					...
					<!--<h4>Company and Family History</h4><hr width="910px" align="right">
						<?php
							$cont = 0;
							$cont1 = 1;
							...
							<a href="#" data-toggle="modal" data-target=".modal-family-<?= $key ?>">
							<img class="images-portfolio" src="updater/site_files/<?= $portfolioFamily->GetCampoValue($key, 'image') ?>" alt="" width="150" height="200">
							</a>
							<?php
							}
						?>-->
			...
			<div class="container">
				...
				<form method="post" action="">
					...
					<div class="row">
						...
						<div class="col-md-4"><center><input type="submit" class="btn btn-lg btn-primary btn-custom" tabindex="4" value="Send" tabindex="3" required></center></div>
						<div class="col-md-4">&nbsp;</div>
					</div>
				</form>
			</div>
